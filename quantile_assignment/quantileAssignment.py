...
from .bipartiteMatchingLibrary import *
...
def calculate_affinity(features_a,
    ...
    logging.debug(f"Expected affinity size: {affinity_shape}")
...
def find_k_alpha(affinity_shape,
    ...
    # As in paper; ratio of unmatched and bad matches over all possibilities
        ...
        logging.debug(f"Alpha min/max {alpha_min}/{alpha_max}")
    ...
    logging.debug(f"Alpha is {alpha} with k_alpha {k_alpha}")
...
def binary_search(q_list, k_alpha, affinity_matrix, matching_method,
    ...
    while left <= right:
        ...
        logging.debug(f"Current mid is {mid_q} with cost {cost}")
